...
@onready var skull_sprite: Node2D = $AnimatedSprite2D

@export var shadow_base_scale := Vector2(1.6, 0.45)
@export var shadow_squash := 0.25   # 0.0–0.4
@export var shadow_base_alpha := 0.35
...
func die() -> void:
    if _dead: return
    _dead = true
    # Important: différer pour ne pas modifier Area2D pendant les signaux
    call_deferred("_die_impl")
...
func _die_impl() -> void:
    ...
    # Pas de respawn → mort définitive
    if !_debug_respawn:
    ...
    if is_instance_valid(sprite):
        sprite.visible = true
        # Échelle non cumulative (revient à la base puis applique le multiplicateur)
        sprite.scale = _base_scale * _debug_respawn_scale_mult
...
# --- Orientation visuelle (flip gauche/droite) ---
    # On retourne le sprite quand il se déplace vers la gauche.
    # Le seuil évite le flip quand la vitesse est ~0.
    var face_threshold: float = 5.0
    if absf(velocity.x) > face_threshold and is_instance_valid(sprite):
        sprite.flip_h = velocity.x < 0.0
    elif is_instance_valid(_player) and is_instance_valid(sprite):
        # Si quasi immobile, oriente vers le joueur (optionnel)
